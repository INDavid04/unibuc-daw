...
/// Left join intrucat vrem sa luam in considerare si evenimentele fara click-uri
$stmt = $db->prepare("
    select e.denumire, count(t.id_traseu) as vizionari
    from eveniment e
    left join traseu t on e.id_eveniment = t.id_eveniment
    group by e.id_eveniment, e.denumire
    order by vizionari desc
    limit 10
");
$stmt->execute();
$vizionari = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>IND | DAW Analiza traseu</title>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            /// Daca nu avem vizionari afisam un mesaj
            <?php
                if (empty($vizionari)) {
                    ?>
                        document.getElementById('chart_div').innerHTML = '<p>Inca nu au fost vizionate evenimente</p>';
                        return;
                    <?php
                }
            ?>
            ...
            let data = google.visualization.arrayToDataTable([
                ['Eveniment', 'Vizionari']
                <?php
                    foreach ($vizionari as $row) {
                        echo ", ['" . addslashes($row['denumire']) . "', " . (int)$row['vizionari'] . "]";
                    }
                ?>
            ]);

            let options = {
                title: 'Top #10 cele mai cautate evenimente',
                titleTextStyle: { 
                    color: '#faebd7', 
                    fontSize: 16 
                },
                hAxis: { 
                    title: 'Numar vizionari', 
                    titleTextStyle: { color: '#faebd7' }, 
                    textStyle: { color: '#faebd7' } 
                }, 
                vAxis: { 
                    title: 'Evenimente', 
                    titleTextStyle: { color: '#faebd7' }, 
                    textStyle: { color: '#faebd7' }, 
                    gridlines: { color: '#faebd7' },
                    textPosition: 'out'
                },
                colors: ['#00bfff'],
                backgroundColor: '#333',
