...
try {
    ...
    /// Verifica daca a fost gasit biletul
    if (!$bilet) {
        throw new Exception ("Nu am gasit biletul #$id_bilet");
    }

    /// Verifica ca biletul sa apartina utilizatorului
    if ($bilet['id_spectator'] != $_SESSION['id_utilizator']) {
        throw new Exception("Nu puteti gestiona un bilet care nu este al dumneavoastra");
    }
    ...
    /// Salveaza pdf-ul in pdf_content
        ...
        <?php
            if ($eroare) {
                ...
                try {	
                    ...
                    $mail->SMTPAuth   = true; 
                    $mail->Username   = 'bilet@dirimia.daw.ssmr.ro';
                    $mail->Password   = 'bilet1234';
                    $mail->SMTPSecure = "ssl";
                    $mail->Port       = 465;  
                    $mail->SMTPDebug  = 3;   

                    /// Expeditor
                    $mail->AddReplyTo('sesizari-bilete@dirimia.daw.ssmr.ro'); /// bilet1234
                    $mail->From = ('bilet@dirimia.daw.ssmr.ro');
                    $mail->SetFrom('bilet@dirimia.daw.ssmr.ro', 'DAW Organizare Evenimente | Bilete');

                    /// Destinatar
                    $mail->AddAddress($_POST['mail']);

                    /// Attachement
                    $mail->addStringAttachment($pdf_content, 'bilet-' . $id_bilet . '.pdf');
                    ...
                    $phpmailerError = $e->errorMessage();
                }

                if ($phpmailerError) {
                    echo "Eroare: " . $e->errorMessage();
                } else {
                    echo "Am trimis biletul pe mail la " . $_POST['mail'];
                }
            }
        ?>
